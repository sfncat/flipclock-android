...
- **Custom fonts**: The info bar and weather display can each use a different font. 10 open-source CJK fonts are bundled (LXGW Marker Gothic, LXGW Neo XiHei, LXGW Neo ZhiSong, LXGW WenKai Mono GB Lite, LXGW XiHei MN, LXGW ZhenKai GB, Smiley Sans, WenQuanYi Micro Hei, Xiaolai, Yozai). The info bar defaults to LXGW XiHei MN; weather defaults to LXGW Neo ZhiSong.
...
- The app bundles 10 open-source CJK fonts (in `assets/fonts/`); the info bar and weather display can each use a different font, all licensed under the SIL Open Font License 1.1.
