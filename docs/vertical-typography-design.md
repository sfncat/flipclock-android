...
当前 FlipClock 在竖屏（`clock->w < clock->h`）时，信息栏为屏幕左侧的竖向长条，但其中的日期、星期、农历仍采用「横向文字、多行堆叠」的方式（[`info_bar.c`](file:///c:/workspace/PhotosFrame/ref/flipclock-android/app/jni/flipclock/srcs/info_bar.c#L272-L320) 的 `_draw_vertical`）。用户希望在竖屏时改用**传统竖排排版**（字符自上而下逐字排列），使界面更贴合中文日历/挂钟的观感。
...
| 布局刷新 | `SDL_WINDOWEVENT_SIZE_CHANGED` → `_flipclock_clock_update_layout()`（[`clock.c`](file:///c:/workspace/PhotosFrame/ref/flipclock-android/app/jni/flipclock/srcs/clock.c#L382-L396)），旋转屏幕后自动重排，无需额外处理 |
...
[`clock.c`](file:///c:/workspace/PhotosFrame/ref/flipclock-android/app/jni/flipclock/srcs/clock.c#L89-L135) 中竖屏分支：
...
[`info_bar.c`](file:///c:/workspace/PhotosFrame/ref/flipclock-android/app/jni/flipclock/srcs/info_bar.c#L272-L320) 的 `_draw_vertical`：

- 日期按空格/`-` 拆行（`_split_into_lines`），如 `2026` / `08-04`；
- 星期、农历各一行；
- 各行为横向文本，逐行堆叠，垂直居中、水平对齐到最宽行。

设计文档 [`date-info-design.md`](file:///c:/workspace/PhotosFrame/ref/flipclock-android/docs/date-info-design.md#L127-L138) 第 5.3 节明确注明：第一版采用「横向文字、多行堆叠」，竖排留待后续。
...
```
                    ┌─ info_vertical = true ─┐
竖屏 (w < h) ───────┤                        ├─ 传统竖排（本次新增 _draw_vertical_typography）
                    └─ info_vertical = false ┘
                                                 └─ 横向文字多行堆叠（现有 _draw_vertical，保留）
横屏 (w >= h) ────── 固定横向布局（现有 _draw_horizontal，info_vertical 不生效）
```

- 新增配置键 `info_vertical`（bool，默认 `false`，保持现有行为）；
- 竖排仅在竖屏时生效；横屏始终横向布局；
...
| 键 | 类型 | 默认 | 说明 |
|----|------|------|------|
| `info_vertical` | bool | `false` | 竖屏时信息栏采用传统竖排文字；`false` 为横向文字多行堆叠 |
...
3. **字号**（在现有 `flipclock_info_bar_set_rect` 中计算，竖排分支）：
   - 宽度约束：`px_w = rect.w * 0.9`（竖排只需一列宽度）；
   - 高度约束：直立字符占用约 `px`，旋转字符占用约 `0.6 * px`，组间空隙 `gap = 0.6 * px`；令
     `units = cjk_count + ascii_count * 0.6 + (g - 1) * 0.6`，则 `px_h = rect.h / units`；
   - `px = min(px_w, px_h) * info_scale`，下限 `8`（与现有下限一致）；
4. **绘制**：
   - 计算总高度 `total_h` 与最宽列宽 `col_w`；
   - 垂直居中：`start_y = rect.y + (rect.h - total_h) / 2`；
   - 水平居中：`x = rect.x + (rect.w - col_w) / 2`；
   - 依序绘制每个 glyph，组间额外插入 `gap` 空隙；
...
- `setChecked(prefs.getBoolean(KEY_INFO_VERTICAL, false))`；
...
在「显示农历年份」开关行（[第 126-145 行](file:///c:/workspace/PhotosFrame/ref/flipclock-android/app/src/main/res/layout/activity_settings.xml#L126-L145)）之后，仿照现有 `LinearLayout + Switch` 结构新增一行：
...
[`AndroidManifest.xml`](file:///c:/workspace/PhotosFrame/ref/flipclock-android/app/src/main/AndroidManifest.xml#L89) 中 MainActivity：
...
- native 层收到 `SDL_WINDOWEVENT_SIZE_CHANGED` 后自动重排（[`clock.c`](file:///c:/workspace/PhotosFrame/ref/flipclock-android/app/jni/flipclock/srcs/clock.c#L382-L396)），无需额外代码。
...
| 旧版本已保存 `show_*` 偏好 | 不受影响；`info_vertical` 默认 `false`，行为与升级前一致 |
...
- [ ] `flipclock.h`：`struct flipclock` 新增 `bool info_vertical;`
- [ ] `flipclock.c`：默认值 `false`；`_flipclock_apply_key_value` 解析 `info_vertical`
- [ ] `info_bar.c`：新增 `_utf8_decode_char` / `_is_cjk` / `_render_glyph` / `_draw_vertical_typography`
- [ ] `info_bar.c`：`flipclock_info_bar_set_rect` 竖排字号分支
- [ ] `info_bar.c`：`flipclock_info_bar_draw` 分派逻辑
- [ ] `MainActivity.java`：`writeNativeConf()` 输出 `info_vertical`
- [ ] `SettingsActivity.java`：`KEY_INFO_VERTICAL` + 开关读写
- [ ] `strings.xml`：`info_vertical_label`
- [ ] `activity_settings.xml`：新增竖排开关行
- [ ] `AndroidManifest.xml`：`sensorLandscape` → `fullSensor`
- [ ] 构建 + 真机验证（第 11 节）
