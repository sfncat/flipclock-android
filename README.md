...
- **日期 / 星期 / 农历显示**：默认开启，可在设置页单独开启 / 关闭；横屏显示在主界面顶部，竖屏显示在主界面左侧，字体小于时间。农历年份显示干支+生肖（如丙午马年）。
- **节气 / 三伏 / 九九显示**：当天为节气日时显示节气名（如白露、大暑）；三伏期内显示初伏/中伏/末伏第X天；九九期内显示一九~九九第X天；节气与三伏/九九同日时两者都显示（如大暑 初伏第九天）。使用 1900–2100 离线节气表，无需网络。
- **双行 / 双列信息栏**：开启节气功能后，横屏自动切换为双行（第1行日期+星期+天气，第2行农历+节气），竖屏切换为双列（左列农历+节气，右列星期+天气），每区独立字号自适应，字体尽量大。无节气内容时回退单行/单列。
...
- **天气显示**：可在设置页开启，在主界面显示当前天气（地点、温度、天气状况）。支持自定义地点输入和地点验证。横屏时天气一律并入信息栏。天气刷新间隔可在 1–24 小时之间调节，默认 1 小时。
- **竖屏日期置顶（实验）**：竖屏时把「年月日」移到最上方（下方左侧为信息栏、右侧为时间），防烧屏改为左右移动 + 淡入淡出，避免竖排 90° 旋转阅读疲劳。默认关闭，修改后需重启应用生效。
- **自定义字体**：信息栏和第二排（横屏第二行/竖屏右列）可分别使用不同字体，内置 10 款开源中文字体可供选择（源雲明體、未来荧黑、鸿蒙黑体、霞鹜漫黑、得意黑、獅尾半月、小赖字体、悠哉字体、站酷快乐体、Z工坊像素黑体）。信息栏默认使用未来荧黑，第二排默认使用霞鹜漫黑。
...
4. （可选）在设置页的 **主界面显示内容** 中单独开启 / 关闭 **日期 / 星期 / 农历 / 农历年份**（默认均开启），更改后需重启 FlipClock 才能生效。横屏时这三项显示在主界面顶部，竖屏时显示在主界面左侧，默认采用传统竖排文字（关闭「竖排文字」可切换回横向堆叠）。
5. （可选）在设置页的 **天气** 区域开启 **显示天气**，输入地点名称（如「北京」），点击 **验证地点** 确认地点有效。可调节刷新间隔（默认 1 小时）。
6. （可选）在设置页的 **信息栏字体** 和 **第二排字体** 中分别选择字体。信息栏字体用于横屏第一行和竖屏左列；第二排字体用于横屏第二行（农历+节气）和竖屏右列（星期+天气）。
...
| **显示星期** | 在主界面信息栏显示当前星期。 |
| **显示农历** | 在主界面信息栏显示农历。 |
| **显示农历年份** | 在农历信息中同时显示干支年+生肖（如丙午马年）。默认开启。 |
| **节气双行信息栏** | 开启后横屏双行、竖屏双列显示节气/三伏/九九；无节气内容时回退单行/单列。默认开启。 |
| **竖排文字** | 竖屏时信息栏采用传统竖排排版；关闭后切换为横向堆叠多行。 |
| **竖屏日期置顶** | 竖屏时把「年月日」移到最上方，下方左侧为信息栏、右侧为时间。默认关闭。 |
...
| **信息栏字体** | 为信息栏选择自定义字体，默认使用未来荧黑。 |
| **第二排字体** | 为横屏第二行/竖屏右列选择自定义字体，默认使用霞鹜漫黑。 |
...
- 节气按北京时间（UTC+8）日历日计算；设备时区非 UTC+8 时可能与国内年历差一天。
...
- 节气、三伏、九九由 `solar_terms.c` 渲染，使用 1900–2100 离线节气表（201×24），三伏按庚日算法、九九按冬至起算，与 `test/almanac/almanac_ref.py` Python 参考实现逐天对拍一致（73414 天全量回归）。
- 应用内置 10 款开源中文字体（位于 `assets/fonts/`，均为子集化裁剪后的字体），信息栏和第二排可分别使用不同字体，遵循 SIL Open Font License 1.1 开源协议。新增字体需放入 `fonts/` 完整原字体，用 `fonts/subset_fonts.py` 按 `fonts/sub.txt` 字符集裁剪后放入 `assets/fonts/`，并在 `SettingsActivity.FONT_NAME_MAP` 补充显示名。
- 字体子集化：`fonts/sub.txt` 为运行时实际使用的字符集（C 字符串字面量 + strings.xml + Java 字符串），`fonts/subset_fonts.py` 用 fontTools 裁剪，10 款字体从 109.6 MB 裁剪至约 1.3 MB。
- 验证体系：`test/almanac/` 包含 Python oracle（`almanac_ref.py`）、C 对拍 harness（`almanac_harness.c`）、五道回归检查（`check_almanac.py`），覆盖表一致、表合理、实现一致、区间不变、公开锚点。
- 调试：可通过 `adb shell settings put global flipclock.almanac_today yyyy-MM-dd` 伪造"今天"测试节气/三伏/九九显示，测试完 `adb shell settings delete global flipclock.almanac_today` 还原。
